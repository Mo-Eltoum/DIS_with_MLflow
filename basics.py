...
def f1score_torch(pd, gt):
    ...
    return torch.reshape(precision, (1, precision.shape [0])), torch.reshape(recall,
                                                                             (1, recall.shape [0])), torch.reshape(f1, (
    1, f1.shape [0]))


def iou_torch(pd, gt):
    intersection = torch.sum((pd > 128) & (gt > 128))
    union = torch.sum((pd > 128) | (gt > 128))
...
def f1_mae_torch(pred, gt, valid_dataset, idx, mybins, valid_out_dir):
    ...
    return pre.cpu().data.numpy(), rec.cpu().data.numpy(), f1.cpu().data.numpy(), mae.cpu().data.numpy(), iou.cpu().data.numpy()
